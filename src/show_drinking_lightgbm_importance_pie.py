...
def visualize_drinking_lightgbm_importance_pie(model_path, data_path, output_path, top_n=10):
    """
    저장된 고위험 음주 LightGBM 모델을 불러와 변수 중요도를 원형 그래프로 시각화하고 저장합니다.
    """
    ...
    plt.title(f'LightGBM 고위험 음주 예측 모델 변수 중요도 (상위 {top_n}개)', fontsize=16)
    plt.axis('equal')
...
if __name__ == '__main__':
    visualize_drinking_lightgbm_importance_pie(
        model_path='results/drinking/drinking_model_lightgbm.joblib',
        data_path='analysis_data.csv',
        output_path='results/drinking/lightgbm_drinking_importance_pie.png'
    )
