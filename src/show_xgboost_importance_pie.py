...
def visualize_xgboost_importance_pie(model_path, data_path, output_path, top_n=10):
    """
    저장된 XGBoost 모델을 불러와 변수 중요도를 원형 그래프로 시각화하고 저장합니다.
    """
    plt.rcParams['font.family'] = 'Malgun Gothic'
    plt.rcParams['axes.unicode_minus'] = False

    print(f"'{model_path}'에서 XGBoost 모델을 불러옵니다...")
    try:
    ...
    plt.title(f'XGBoost 흡연 예측 모델 변수 중요도 (상위 {top_n}개)', fontsize=16)
    plt.axis('equal')
...
if __name__ == '__main__':
    visualize_xgboost_importance_pie(
        model_path='results/smoking/smoking_model_xgboost.joblib',
        data_path='analysis_data.csv',
        output_path='results/smoking/xgboost_feature_importance_pie.png'
    )
